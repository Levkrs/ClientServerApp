...
SERVER_FILE_NAME = 'test_server.py'
CLIENT_FILE_NAME = 'test_client.py'
...
# client_count = input('Введите требуемое колличество клиентов: ')
...
def stop_server():
    ...
    print('----')
    # print(SERVER_PROCESE[0])
    print(SERVER_PROCESE[0].pid)

    # SERVER_PROCESE[0].kill()
    # SERVER_PROCESE.pop(0)
        ...
        try:
            if os.name == 'posix':
                client = subprocess.Popen(f'osascript -e \'tell application "Terminal" to do'
                                          f' script "python3 {PATH_TO_CLIENT}"\'', shell=True)
                print(client.pid)
            else:
